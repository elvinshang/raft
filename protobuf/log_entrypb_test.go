...
func BenchmarkLogEntryProtoUnmarshal(b *testing.B) {
	...
	for i := 0; i < 10000; i++ {
		dAtA, err := github_com_golang_protobuf_proto.Marshal(NewPopulatedLogEntry(popr, false))
		if err != nil {
			panic(err)
		}
		datas[i] = dAtA
...
func TestLogEntryVerboseEqual(t *testing.T) {
...
func TestLogEntryGoString(t *testing.T) {
	popr := math_rand.New(math_rand.NewSource(time.Now().UnixNano()))
	p := NewPopulatedLogEntry(popr, false)
	s1 := p.GoString()
	s2 := fmt.Sprintf("%#v", p)
...
func TestLogEntryStringer(t *testing.T) {
	popr := math_rand.New(math_rand.NewSource(time.Now().UnixNano()))
	p := NewPopulatedLogEntry(popr, false)
	s1 := p.String()
	s2 := fmt.Sprintf("%v", p)
